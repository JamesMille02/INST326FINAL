...
def compute_null_bed(csv_file):
    ...
    try:
        ...
        predicted_bed_values_rounded = np.round(predicted_bed_values)

        # update the 'BEDS' column with the rounded predicted values
        df.loc[df['BEDS'].isnull(), 'BEDS'] = predicted_bed_values_rounded

        # saves it to the csv
        df.to_csv(csv_file, index=False)

    # if it fails print an error has occurred.
